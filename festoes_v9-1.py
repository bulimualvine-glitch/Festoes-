...
DB_FILE           = "festoes_v9_1.db"   # SQLite DB (v9.1)
...
def db_save_history(h):
    """Save last 200 history entries to SQLite."""
    try:
        con = sqlite3.connect(DB_FILE)
        cur = con.cursor()
        ...
        for entry in h[-200:]:
            cur.execute(
                "INSERT INTO history (sender, text, timestamp, intent) VALUES (?,?,?,?)",
                (entry.get("sender",""), entry.get("text",""),
                 entry.get("timestamp",""), entry.get("intent","unknown"))
            )
        con.commit()
        con.close()
    except: pass
...
def db_load_history():
    """Load history from SQLite. Falls back to JSON if DB missing."""
    try:
        con = sqlite3.connect(DB_FILE)
        cur = con.cursor()
        cur.execute("SELECT sender, text, timestamp, intent FROM history ORDER BY id")
        rows = cur.fetchall()
        con.close()
        if rows:
            return [{"sender": r[0], "text": r[1],
                     "timestamp": r[2], "intent": r[3]} for r in rows]
    except: pass
    return load_history()  # fall back to JSON
...
def db_load_settings():
    """Load all settings from SQLite as dict."""
    try:
        con = sqlite3.connect(DB_FILE)
        cur = con.cursor()
...
def db_save_learned(input_text, response_text):
    """Save a learned pair to SQLite."""
    try:
        con = sqlite3.connect(DB_FILE)
        cur = con.cursor()
        cur.execute(
            "INSERT INTO learned_pairs (input, response, timestamp) VALUES (?,?,?)",
...
# ── v9.1 Upgrade 4: Auto-backup ───────────────────────────────
BACKUP_DIR = "/sdcard/Festoes/backups"
...
def ensure_backup_dir():
    ...
    try:
        ...
        # Fallback to local folder if /sdcard not accessible
        try:
            os.makedirs("festoes_backups", exist_ok=True)
            return "local"
        except:
            return False
...
def auto_backup():
    """
    Backup weights, DB, and history to /sdcard/Festoes/backups/
    Keeps last 5 backups - oldest deleted automatically.
    """
    try:
        bdir = ensure_backup_dir()
        if not bdir:
            return "Backup failed - no write access"

        backup_path = BACKUP_DIR if bdir is True else "festoes_backups"
        stamp = datetime.datetime.now().strftime("%Y%m%d_%H%M")
        dest  = os.path.join(backup_path, f"festoes_{stamp}")
        os.makedirs(dest, exist_ok=True)

        # Files to backup
        files = [
            WEIGHTS_FILE, MICRO_WEIGHTS_FILE, MICRO_VOCAB_FILE,
            HISTORY_FILE, DB_FILE, SETTINGS_FILE, LEARN_FILE
        ]
        backed = 0
        for f in files:
            if os.path.exists(f):
        ...
        try:
            all_backups = sorted([
                ...
                and d.startswith("festoes_")
            ])
            while len(all_backups) > 5:
                old = os.path.join(backup_path, all_backups.pop(0))
                shutil.rmtree(old, ignore_errors=True)
        except: pass

        return f"Backup saved: {dest} ({backed} files)"
    except Exception as e:
        return "Backup error: " + str(e)[:60]
...
def ask_gemini(user_text, mood="calm", mood_conf=0.5, intent="unknown"):
    ...
    try:
        ...
        code_num = e.code
        if code_num == 429:
            # Exponential backoff - retry up to 3 times before giving up
            for _retry in range(3):
                _wait = 2 ** (_retry + 1)   # 2s, 4s, 8s
                _time.sleep(_wait)
                try:
                    with urllib.request.urlopen(req, timeout=25, context=ctx) as _r:
                        _d = json.loads(_r.read().decode())
                        return _d["candidates"][0]["content"]["parts"][0]["text"]
                except urllib.error.HTTPError as _he:
                    if _he.code != 429: break
                except: break
...
def _save_weather_cache(city, result):
    """Cache successful weather result for offline use."""
    try:
        try:
            with open(WEATHER_CACHE) as f: cache = json.load(f)
        except: cache = {}
        cache[city.lower()] = {
            "result":    result,
            "timestamp": datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
        }
        with open(WEATHER_CACHE,"w") as f: json.dump(cache,f)
    except: pass

def _load_weather_cache(city):
    """Load cached weather for a city."""
    try:
        with open(WEATHER_CACHE) as f: cache = json.load(f)
        entry = cache.get(city.lower())
        if entry:
            return entry["result"] + "\n\n(Cached at " + entry["timestamp"] + " - offline)"
    except: pass
    return None
...
def fetch_weather(city):
    try:
        ...
        result = ("Weather in "+d["name"]+", "+d["sys"]["country"]+":\n\n"
        ...
        # v9.1 - cache successful result for offline use
        _save_weather_cache(city, result)
        return result
    except:
        # v9.1 - try offline cache before giving up
        cached = _load_weather_cache(city)
...
# ── v9.1 Upgrade 5: Skills Plugin System ─────────────────────
SKILLS_DIR = "skills"
_loaded_skills = {}

def load_skills():
    """Auto-discover and load skill plugins from /skills folder."""
    global _loaded_skills
    if not os.path.exists(SKILLS_DIR):
        try: os.makedirs(SKILLS_DIR)
        except: pass
        return
        ...
        if fname.endswith(".py") and not fname.startswith("_"):
            ...
            try:
                ...
                spec = importlib.util.spec_from_file_location(
                    skill_name,
                    os.path.join(SKILLS_DIR, fname)
                )
                mod = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(mod)
                _loaded_skills[skill_name] = mod
            except Exception as e:
                pass  # bad skill - skip silently

def run_skill(skill_name, text):
    """Run a loaded skill plugin and return its response."""
    if skill_name in _loaded_skills:
        mod = _loaded_skills[skill_name]
        if hasattr(mod, "handle"):
            try:
                return mod.handle(text)
            except Exception as e:
                return f"Skill error ({skill_name}): {str(e)[:60]}"
    return None

# ── python-chess skill (built-in, no /skills folder needed) ───
_chess_lib = None

def _load_chess_lib():
    """Lazy-load python-chess. Install: pip install chess"""
    global _chess_lib
    if _chess_lib is not None:
        return _chess_lib
    try:
        import chess
        import chess.pgn
        import io
        _chess_lib = (chess, chess.pgn, io)
    except ImportError:
        _chess_lib = False
    return _chess_lib

def analyse_chess_position(text):
    """
    Analyse a chess position or PGN using python-chess.
    Returns board state, legal moves count, game phase.
    Falls back to Gemini if python-chess not installed.
    """
    libs = _load_chess_lib()
    if not libs:
        return None  # python-chess not installed - Gemini will handle it

    chess_mod, pgn_mod, io_mod = libs
    tl = text.lower()

    try:
        # Try to parse as FEN position
        fen_match = re.search(
            r'[rnbqkpRNBQKP1-8]{1,8}/[rnbqkpRNBQKP1-8/]{10,}', text)
        if fen_match:
            board = chess_mod.Board(fen_match.group())
            moves = list(board.legal_moves)
            phase = "endgame" if len(board.piece_map()) < 14 else "middlegame"
            result = (
                f"Position analysis:\n"
                f"  Turn: {'White' if board.turn else 'Black'}\n"
                f"  Legal moves: {len(moves)}\n"
                f"  Phase: {phase}\n"
                f"  In check: {board.is_check()}\n"
                f"  Checkmate: {board.is_checkmate()}\n"
                f"  Stalemate: {board.is_stalemate()}"
            )
            return result

        # Try to parse as PGN
        pgn_text = text if "1." in text else None
        if pgn_text:
            game = pgn_mod.read_game(io_mod.StringIO(pgn_text))
            if game:
                board = game.board()
                moves_list = []
                ...
                result = (
                    f"PGN analysis (python-chess):\n"
                    f"  Moves played: {len(moves_list)}\n"
                    f"  Opening: {opening}\n"
                    f"  Final position check: {board.is_check()}\n"
                    f"  Moves: {' '.join(moves_list[:10])}"
                    + (" ..." if len(moves_list) > 10 else "")
                )
                return result
    except Exception as e:
...
# ── v9.1 Upgrade 1: Int8 Weight Quantization ──────────────────
def quantize_weights(W):
    """Quantize float32 weights to int8 - 4x faster inference on mobile."""
    scale = np.max(np.abs(W)) / 127.0 + 1e-8
    return np.clip(np.round(W / scale), -128, 127).astype(np.int8), scale

def dequantize_weights(W_int8, scale):
    """Restore int8 weights to float32 for computation."""
    return W_int8.astype(np.float32) * scale

def quantize_network(net):
    """Quantize all weight matrices in a micro-network."""
    W1, W2, W3, b1, b2, b3 = net
    W1q, s1 = quantize_weights(W1)
    W2q, s2 = quantize_weights(W2)
    W3q, s3 = quantize_weights(W3)
    return (W1q, W2q, W3q, b1, b2, b3, s1, s2, s3)

def run_micro_network_quantized(text, net_q):
    """Run inference on a quantized micro-network."""
    W1q, W2q, W3q, b1, b2, b3, s1, s2, s3 = net_q
    x   = micro_vectorize(text)
    if x.shape[1] != W1q.shape[0]: return 0, 0.25
    W1f = dequantize_weights(W1q, s1)
    W2f = dequantize_weights(W2q, s2)
    W3f = dequantize_weights(W3q, s3)
    h1  = rms_norm(np.maximum(0, x.dot(W1f) + b1))
    h2  = np.maximum(0, h1.dot(W2f) + b2)
    raw = h2.dot(W3f) + b3
...
# Global flag - set True after quantization completes
_NETWORKS_QUANTIZED = False
AI_NETWORKS_Q = {}  # quantized versions of AI_NETWORKS

def quantize_all_networks():
    """Quantize all 1000 micro-networks in background thread."""
    global _NETWORKS_QUANTIZED, AI_NETWORKS_Q
    try:
        for name, net in AI_NETWORKS.items():
            if len(net) == 6:   # unquantized (W1,W2,W3,b1,b2,b3)
                AI_NETWORKS_Q[name] = quantize_network(net)
        _NETWORKS_QUANTIZED = True
    except Exception as e:
        _NETWORKS_QUANTIZED = False  # fall back to float32
...
def run_120_brain(text, intent="unknown"):
    ...
    outputs = []
    for name in sample:
        if _NETWORKS_QUANTIZED and name in AI_NETWORKS_Q:
            idx, conf = run_micro_network_quantized(text, AI_NETWORKS_Q[name])
        else:
...
def learn_from_pair(user_text, gemini_response):
    ...
    v9.1: Also saves to SQLite DB.
    """
    # v9.1 - save to DB
    db_save_learned(user_text, gemini_response)
    global AI_NETWORKS, MICRO_VOCAB
...
class FestoesApp:
    ...
    def _build_ui(self):
        ...
        self.lbl_title = tk.Label(hdr, text="Festoes v9.1",
            font=FONT_TITLE, fg=FG_WHITE, bg=BG_PANEL)
        self.lbl_title.pack(side=tk.LEFT, padx=10)

        self.lbl_badge = tk.Label(hdr, text="Gemini + CSV + v9.1 Brain",
        ...
        for lbl,cmd in [("Friend","be my friend"),("Casual","casual mode"),
                        ...
                        ("Agent","agent status"),("Backup","backup now")]:
        ...
        # v9.1 Markdown tags
        self.chat.tag_configure("md_h1",     foreground="#FFFFFF", font=(FONT_MSG[0], FONT_MSG[1]+4, "bold"))
        self.chat.tag_configure("md_h2",     foreground="#e0e0e0", font=(FONT_MSG[0], FONT_MSG[1]+2, "bold"))
        self.chat.tag_configure("md_h3",     foreground="#c0c0c0", font=(FONT_MSG[0], FONT_MSG[1]+1, "bold"))
        self.chat.tag_configure("md_bold",   foreground="#FFFFFF", font=(FONT_MSG[0], FONT_MSG[1],   "bold"))
        self.chat.tag_configure("md_italic", foreground="#e0e0e0", font=(FONT_MSG[0], FONT_MSG[1],   "italic"))
        self.chat.tag_configure("md_bullet", foreground="#b0c4de", font=FONT_MSG)
        self.chat.tag_configure("md_code",   foreground="#98FB98", font=("Courier", FONT_MSG[1]-1))
    ...
    def _startup(self):
        name = settings.get("username","Alvine")
        # v9.1 - init DB and backup dir on startup
        db_init()
        ensure_backup_dir()
        load_skills()
        self._sys_msg(f"Festoes v9.1 online. Welcome back, {name}!")
        threading.Thread(target=self._health_check, daemon=True).start()
        threading.Thread(target=self._train, daemon=True).start()

    def _health_check(self):
        """v9.1 Upgrade 8 - startup health check."""
        issues  = []
        notices = []

        # Check critical files
        if not os.path.exists(WEIGHTS_FILE):
            notices.append("No weights file - will train on first run")
        if not GEMINI_API_KEY:
            issues.append("No Gemini API key set")
        if not WEATHER_API_KEY:
            issues.append("No weather API key set")
        if not GOOGLE_SEARCH_API_KEY:
            notices.append("Google Search key not set - using DuckDuckGo")

        # Check python-chess availability
        chess_ok = _load_chess_lib() is not False
        notices.append("python-chess: " + ("installed" if chess_ok else "not installed (pip install chess)"))

        # Check /sdcard access for backups
        sdcard_ok = os.path.exists("/sdcard")
        notices.append("Backup dir: " + (BACKUP_DIR if sdcard_ok else "festoes_backups/ (local)"))

        # Check DB
        db_ok = db_init()
        notices.append("SQLite DB: " + ("ready" if db_ok else "failed - using JSON fallback"))

        # Check network (ping Gemini endpoint)
        net_ok = False
        try:
            ctx = ssl.create_default_context()
            ctx.check_hostname = False
            ...
            urllib.request.urlopen(
                "https://generativelanguage.googleapis.com", timeout=4, context=ctx)
            net_ok = True
        except: pass
        notices.append("Network: " + ("online" if net_ok else "offline - local brain active"))

        # Quantize networks in background after load
        if AI_NETWORKS:
            quantize_all_networks()
            notices.append("Int8 quantization: complete (" + str(len(AI_NETWORKS_Q)) + " networks)")

        # Display health report
        report = "=== v9.1 Health Check ===\n"
        if issues:
            report += "WARNINGS:\n" + "\n".join("  ! " + i for i in issues) + "\n"
        report += "\n".join("  + " + n for n in notices)
        self.root.after(500, lambda r=report: self._sys_msg(r))
    ...
    def _train(self):
        if weights_exist() and micro_weights_exist():
            ...
            load_all_weights()
            load_micro_weights()
            # Quantize in background after load
            threading.Thread(target=quantize_all_networks, daemon=True).start()
            ...
            self.root.after(0, lambda: self.status.config(
                text="Trained! " + str(len(MICRO_VOCAB)) + " words | " +
                     str(len(AI_NETWORKS)) + " networks"))
            self.root.after(0, lambda: self._sys_msg(
                "v9.1 training complete! CSV + 1,000 NNs + Int8 quantization running." +
    ...
    def process(self, text, image_path=None, agent=False):
        ...
        # ── Personality switch ──
        # ── Backup command ──
        if tl in ("backup", "backup now", "save backup", "create backup"):
            self.root.after(0, lambda: self.status.config(text="Backing up..."))
            result = auto_backup()
            self.root.after(0, lambda r=result: self.finish(r))
            return
        ...
        # ── Chess PGN / position analysis ──
        if detect_pgn(text):
            # Try python-chess first for instant local analysis
            chess_result = analyse_chess_position(text)
            if chess_result:
                self.root.after(0, lambda r=chess_result: self.finish(r))
                return
            # Fall back to Gemini for richer commentary
    ...
    def _render_markdown(self, text):
        """v9.1 Markdown renderer - no external libs needed."""
        segments = []
            ...
            if line.startswith("### "):
                ...
                for part in parts:
                    if part.startswith("**") and part.endswith("**"):
                        segments.append((part[2:-2], "md_bold"))
                    elif part.startswith("*") and part.endswith("*") and len(part) > 2:
                        segments.append((part[1:-1], "md_italic"))
                    elif part.startswith("`") and part.endswith("`") and len(part) > 2:
                        segments.append((part[1:-1], "md_code"))
                    elif part:
                        segments.append((part, "bot_msg"))
                segments.append(("\n", "bot_msg"))
        return segments
    ...
    def finish(self, result, skip_score=False):
        ...
        # v9.1 - render markdown in bot reply
        for seg_text, seg_tag in self._render_markdown(result):
            self.chat.insert(tk.END, seg_text, seg_tag)
